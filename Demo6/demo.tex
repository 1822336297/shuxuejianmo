...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{figures/shashisuanfa.png}
...
由表~\ref{zhuansasgzai}~可知，由沙石算法设计的装备兵力装载方案，其有效面积利用率相当高，能大幅度减少船只数目填充更多武装兵力。
	...
	问题二要求针对具体运输任务，结合港口泊位，制定时间最短、船只数量最少的兵力装载方案。
	本组通过遗传算法，将问题一中的决策向量作为染色体，装载时间和船支数量作为适应度函数，建立装载方案优化模型。修改问题一中的沙石算法，作为验证算法判断每个个体是否可以存活，并随机生成$w$个能通过验证算法的初始个体。将某一代个体经过交叉、变异后的基因带入时间轴仿真模型，计算出装载所用时间作为适应度函数，将生成个体按照适应度函数大小进行第一次排序，再将装载所用时间大小相同的函数进行第二次排序，取出排序中前$w$个个体进行下一次进化，进化$g$代个体后可求得装载时间最短，同时使用海上运输工具最少的兵力装载方案。	                                                                                                    
	...
	\section{问题三模型的建立与求解}
    \subsection{问题描述与分析}
    问题三要求，假设当装载行动开始24小时后，A港和D港有部分泊位被毁，给出调整方案和装载建议。
    
    本组基于问题二中模型求解的结果，根据装载时间最短、使用运输工具最少的装载方案，作出合理假设，假设泊位被毁不损失船只和兵力，装载工作停止且丢失进度。剔除已经完成装载任务的船舰和被毁的码头泊位，将剩余船舰的种类和数量定义为决策向量，装载时间和船支数量作为适应度函数，输入到装载方案优化模型中，通过遗传算法求解出对剩余船舰、码头泊位的装载方案，给出装载建议。
    \subsection{模型的建立与求解}
