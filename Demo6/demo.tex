...
%三线表
	...
	\section{问题一模型的建立与求解}
    \subsection{问题描述与分析}
    问题一要求根据输送任务和船舰情况，结合相关条件和要求，分析运输船舰和兵力的关系，概算各类型旅一个旅级单位对船舰的需求量,给出装载方案。本文首先对兵力、装备与装载的面积进行量化，得到所占面积向量。定义船只的决策向量，并结合登陆舰Y1的有效装载面积大于X9(直升机)及第$Ⅵ$级旅人口面积的约束条件优化函数，求解使用船舰总数最小值，通过xxxxxxxxxxxxx,得到各级旅一个旅级单位包裹对不同种船舰的需求量,问题一算法流程图如图所示：
	    %~\ref{77}~
	    %\begin{figure}[H]
	    %	\centering
