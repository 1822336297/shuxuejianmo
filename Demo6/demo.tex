...
%三线表
	...
	通过量化各型旅成建制单位的人数、装备面积和船舰的有效装载面积，结合相关条件和要求,再运用沙石算法进行计算,求解出各旅级单位装备人口面积装载方案如表~\ref{zhuangzai}~
		...
		\end{tabular}
	\end{table}
	%由表~\ref{zhuangzai}~可得，第Ⅰ型旅的一个旅级单位对船舰的需求为1艘Y1综合登陆舰，3艘Y2大型登陆舰，3艘Y3大型登陆舰，4艘Y4大型登陆舰，11艘Y5大型登陆舰，2艘Y6大型登陆舰和1艘Y8大型登陆舰。(第$Ⅺ$型旅的需求与其为同一艘Y1综合登陆舰)，第Ⅱ型旅的一个旅级单位的需求为8艘Y6中型登陆舰，1艘Y7中型登陆舰，7艘Y9中型登陆舰与1艘集装箱船只。
	
			
	\section{问题二模型的建立与求解}
	\subsection{问题的描述与分析}
	问题二要求针对具体运输任务，结合码头的泊位，制定时间最短、船只数量最少的兵力装载方案。本组通过遗传算法，将问题一中的决策向量作为染色体，装载时间和船支数量作为适应度函数，建立装载方案优化模型。首先修改问题一中的沙石算法，作为判断每个个体是否可以存活的基础。并生成$w$个初始个体
