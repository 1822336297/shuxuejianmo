...
%三线表
    ...
    \subsection{问题描述与分析}
    问题一要求根据输送任务和船舰情况分析运输船舰和兵力的关系，概算各类型旅一个旅级单位对船舰的需求量与装载方案。其本质是一个\textbf{多背包优化}问题：要求使用尽可能少的背包，装载各种旅的一个旅编制兵力。根据题目要求，以派出船舰种类和数量作为决策向量，以所用的总船只数量为目标函数，以每艘船的面积装载限制作为约束条件。通过本组设计的\textbf{沙石算法}，使得每艘船面积利用率达到最高，从而使得调用船支数达到最小值，并给出其装载方案。其沙石算法示意图如图 ~\ref{shashi}~所示：
